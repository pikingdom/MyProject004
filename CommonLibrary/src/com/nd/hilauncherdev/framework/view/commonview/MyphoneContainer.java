...
//		bottomColor = Color.parseColor("#d3d3d3");
     ...
     * 添加底部操作栏(可以没有底部操作栏)
     * @param text
     * @param src (小图标)不能为空,如果不用小图标,设置为<=0 new int[]{-1,-1,-1}
     * @param listeners 可以为空
     */
...
//		tv.setTextColor(bottomColor);// 参见@color/white
    ...
    private RelativeLayout createBottom(String text, Drawable src, OnClickListener listener) {
        ...
        LinearLayout innerLayout = new LinearLayout(mContext);
        innerLayout.setId(199);
        innerLayout.setLayoutParams(new RelativeLayout.LayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT));
        ...
        tv.setTextSize(14);// 参见@dimen/myphone_title_bottom_text_size
//		tv.setTextColor(bottomColor);// 参见@color/white
        tv.setTextColor(getResources().getColor(R.color.white));
        tv.setText(text);
        LayoutParams params = new LayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT);
