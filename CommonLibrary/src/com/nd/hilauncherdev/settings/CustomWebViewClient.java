...
import com.nd.hilauncherdev.kitset.once.TagList;
import com.nd.hilauncherdev.kitset.uconfig.ContentConverter;
import com.nd.hilauncherdev.kitset.uconfig.UConfig;
import com.nd.hilauncherdev.kitset.uconfig.UConfigList;
                    ...
                    if (intent != null) {
                        ...
                        if ("zm91txwd20141231".equals(uri.getScheme()) || "zm91txwd20141231service".equals(uri.getScheme())) {
                            ...
                            if (mSchemeOpt == 0) {//对协议有限制，白名单为Data数组,协议头在白名单中且对应的应用已经安装时提示，否则不提示
                                ...
                                if (ws != null && ApkTools.isApkInstalled(context, ws.packageName)) {
                                    valided = true;
        ...
        public void run() {
            try {
                ...
                try {
                    ...
                    final SharedPreferences sp = BaseConfig.getApplicationContext().getSharedPreferences(BaseConfigPreferences.NAME, 4);
                    if (sp != null) {
...
//                        int ver = jsonObject.optInt("version");
                    ...
                    initSchemeConfig(BaseConfig.getApplicationContext());
                } catch (Exception ex) {
                    ex.printStackTrace();
                } finally {
                    Once.markDone(TagList.TAG_WEB_BASED_INTENT_SCHEMES);
